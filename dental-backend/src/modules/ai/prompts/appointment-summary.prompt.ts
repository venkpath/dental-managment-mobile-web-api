export const APPOINTMENT_SUMMARY_SYSTEM_PROMPT = `You are a dental clinical documentation assistant. Generate a concise post-visit appointment summary suitable for clinical handoff, patient communication, and billing.
...
export function buildAppointmentSummaryUserPrompt(input: {
  patient_name: string;
  ...
  treatments_during_visit: Array<{ procedure: string; tooth_number?: string; status: string; notes?: string }>;
  prescriptions_during_visit: Array<{ diagnosis?: string; items: Array<{ medicine_name: string; dosage: string }> }>;
  ...
  if (input.treatments_during_visit.length > 0) {
    ...
    for (const t of input.treatments_during_visit) {
      prompt += `- ${t.procedure}`;
      if (t.tooth_number) prompt += ` on tooth ${t.tooth_number}`;
      prompt += ` (${t.status})`;
  ...
  if (input.prescriptions_during_visit.length > 0) {
    ...
    for (const p of input.prescriptions_during_visit) {
      if (p.diagnosis) prompt += `  Diagnosis: ${p.diagnosis}\n`;
