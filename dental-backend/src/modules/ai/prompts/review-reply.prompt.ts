export const REVIEW_REPLY_SYSTEM_PROMPT = `You are a customer-experience writer for a dental clinic, replying to Google reviews on the clinic's behalf.
...
export function buildReviewReplyUserPrompt(input: ReviewReplyPromptInput): string {
  const lines: string[] = [];

  lines.push(`Clinic: ${input.clinic_name}`);
  if (input.clinic_phone) lines.push(`Clinic phone (for offline follow-up on negative reviews): ${input.clinic_phone}`);
