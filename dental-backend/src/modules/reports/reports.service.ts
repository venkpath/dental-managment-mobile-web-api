...
export class ReportsService {
  ...
  async getDashboardSparklines(
    ...
    daily: Array<{ date: string; revenue: number; appointments: number; expenses: number }>;
    trends: {
    ...
    const [rawRevenue, rawAppointments, rawExpenses, lastMonthRev, lastMonthExp, lastMonthRef, thisMonthRev, thisMonthExp, thisMonthRef, thisMonthOutstanding, lastMonthOutstanding] =
      await Promise.all([
        this.prisma.$queryRaw<Array<{ day: string; total: number }>>`
        ...
        this.prisma.$queryRaw<Array<{ day: string; total: number }>>`
          ...
          WHERE clinic_id = ${clinicId}::uuid
            ...
            ${branchFilter ? Prisma.sql`AND branch_id = ${branchFilter}::uuid` : Prisma.empty}
            ${dentistId ? Prisma.sql`AND dentist_id = ${dentistId}::uuid` : Prisma.empty}
          GROUP BY 1 ORDER BY 1
        `,
        ...
        // Outstanding invoices created this month (pending/partially_paid net_amount)
        this.prisma.invoice.aggregate({
          _sum: { net_amount: true },
        ...
        // Outstanding invoices created last month
        this.prisma.invoice.aggregate({
          _sum: { net_amount: true },
    ...
    // Returns null only when there's genuinely no prior data to compare against.
    // If prev > 0 always compute a percentage; if prev = 0 and curr > 0 return +100.
    ...
    return {
      ...
      trends: {
        ...
        net_profit_vs_last_month: pct(
          thisMonthRevVal - thisMonthExpVal - thisMonthRefVal,
          lastMonthRevVal - lastMonthExpVal - lastMonthRefVal,
        ),
      },
    };
  }
