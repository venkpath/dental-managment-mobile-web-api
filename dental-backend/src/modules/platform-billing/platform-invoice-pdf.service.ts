...
export class PlatformInvoicePdfService {
  async generate(data: PlatformInvoicePdfData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      // Status badge (right of meta) — paint per status
      {
        ...
        // Colors: PAID = green (existing PAID_BG/FG), OVERDUE = red, others = neutral
        let bg: string = PAID_BG, fg: string = PAID_FG;
        if (data.status === 'overdue') { bg = '#fee2e2'; fg = '#991b1b'; }
        else if (data.status === 'due' || data.status === 'draft') { bg = '#fef3c7'; fg = '#92400e'; }
        else if (data.status === 'cancelled' || data.status === 'refunded') { bg = '#f3f4f6'; fg = '#6b7280'; }
