    ...
    try {
      ...
      for (const clinic of clinics) {
        try {
          ...
          const waTemplate = await this.findSystemOrClinicTemplate(
            clinic.id,
            'dental_noshow_followup',
            'whatsapp',
          );
          ...
          for (const appt of noShowAppts) {
            try {
              ...
              if (channel === MessageChannel.WHATSAPP && !clinicPhone) {
                this.logger.warn(
                  `No-show follow-up skipped for appointment ${appt.id}: branch/clinic phone missing`,
                );
                continue;
              }
              ...
              // WhatsApp requires an approved Meta template outside the 24h session window.
              // Plain text is rejected as a "Re-engagement message".
              if (channel === MessageChannel.WHATSAPP && !templateId) {
                this.logger.warn(
                  `No-show follow-up skipped for appointment ${appt.id}: dental_noshow_followup template not found`,
                );
                continue;
              }
              ...
              await this.communicationService.sendMessage(clinic.id, {
                ...
                channel,
                category: MessageCategory.TRANSACTIONAL,
                template_id: channel === MessageChannel.WHATSAPP ? templateId : (rule.template_id ?? undefined),
                body: channel === MessageChannel.WHATSAPP && templateId ? undefined : fallbackBody,
                variables,
              ...
              this.logger.warn(
                `No-show follow-up failed for appointment ${appt.id}: ${(e as Error).message}`,
              );
            }
          }
        } catch (e) {
