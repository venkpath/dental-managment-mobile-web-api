...
let ConsentPdfService = class ConsentPdfService {
    static { ConsentPdfService_1 = this; }
    async generate(data) {
    ...
    static M = 40;
    get M() { return ConsentPdfService_1.M; }
    ...
    renderPatientCard(doc, data, startY) {
        const M = this.M;
        ...
        const drawKV = (label, value, x, y, w) => {
    ...
    renderProcedureClause(doc, data, startY) {
        if (!data.body.procedure_clause)
            return startY;
        const M = this.M;
        const CW = doc.page.width - M * 2;
    ...
    renderAnaesthesiaOptions(doc, data, startY) {
    ...
    renderSections(doc, data, startY) {
        const M = this.M;
        const CW = doc.page.width - M * 2;
        let y = startY;
    ...
    renderConsentStatement(doc, data, startY) {
        const M = this.M;
        const CW = doc.page.width - M * 2;
    ...
    renderSignatureLines(doc, data, startY) {
        ...
        lines.forEach((kind, idx) => {
            ...
            if ((kind === 'patient' || kind === 'guardian') && data.signature?.signed_at) {
                ...
                doc.fillColor(TEXT_MUTED).font('Helvetica').fontSize(8.5)
                    .text(d.toLocaleString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }), x, lineY + 26, { width: slotW, align: 'center' });
            }
        });
    }
    renderFooter(doc, data) {
        ...
        for (let i = 0; i < range.count; i++) {
            ...
            doc.fillColor(TEXT_FAINT).font('Helvetica').fontSize(7.5)
                .text(`${data.template_title} • v${data.template_version} • ${data.language.toUpperCase()} • Generated ${data.generated_at.toLocaleString('en-IN')}`, M, y, { width: W - M * 2, align: 'left', lineBreak: false })
                .text(`Page ${i + 1} of ${range.count}`, M, y, { width: W - M * 2, align: 'right', lineBreak: false });
        }
    }
};
exports.ConsentPdfService = ConsentPdfService;
exports.ConsentPdfService = ConsentPdfService = ConsentPdfService_1 = __decorate([
    (0, common_1.Injectable)()
], ConsentPdfService);
//# sourceMappingURL=consent-pdf.service.js.map