...
let AutomationController = class AutomationController {
    ...
    async forceScheduleReminders(clinicId, appointmentId) {
        const appointment = await this.prisma.appointment.findUnique({
            where: { id: appointmentId },
        ...
        for (const r of reminders) {
            ...
            try {
                await this.reminderQueue.add('send-appointment-reminder', { appointmentId, clinicId, reminderIndex: r.index, reminderHours: r.hours }, { jobId, delay, removeOnComplete: true, removeOnFail: 100 });
                results.push({ reminderIndex: r.index, reminderHours: r.hours, status: 'scheduled', jobId, firesAt: sendAt.toISOString() });
            }
            catch (e) {
                results.push({ reminderIndex: r.index, reminderHours: r.hours, status: 'failed', error: e.message });
            }
        }
...
__decorate([
    (0, common_1.Post)('queues/appointment-reminders/schedule-now/:appointmentId'),
    (0, roles_decorator_js_1.Roles)(index_js_1.UserRole.ADMIN),
    (0, swagger_1.ApiOperation)({ summary: 'Force-schedule reminders for an appointment NOW and return per-reminder enqueue results (success/failure with error)' }),
    openapi.ApiResponse({ status: 201, type: Object }),
    __param(0, (0, current_clinic_decorator_js_1.CurrentClinic)()),
    __param(1, (0, common_1.Param)('appointmentId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], AutomationController.prototype, "forceScheduleReminders", null);
